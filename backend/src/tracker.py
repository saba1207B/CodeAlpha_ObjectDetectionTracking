...
class ObjectTracker:
    ...
    def __init__(
        ...
        self.class_names: Dict[int, str] = getattr(model, "names", {}) if model else {}
        ...
        if np is not None:
            ...
            self.id_colors = [(0, 255, 0)] * 100

    def reset(self):
        """Clear all tracking state, histories, and unique IDs."""
    ...
    def track_frame(self, frame) -> List[Dict[str, Any]]:
        ...
        for box in boxes:
            ...
            tracked_objects.append({
                ...
                "class_name": cls_name,
                "centroid": (cx, cy),
                "trail_length": len(self.track_history[track_id]) if track_id is not None else 0,
            })
    ...
    def draw_tracks(
        ...
        for obj in tracked_objects:
            ...
            # 3. Formatted Label Badge: "person | ID: 3 | 0.91"
            if show_labels:
                if track_id is not None:
    ...
    def draw_hud(
        ...
        """Draw an informational Heads-Up Display (HUD) overlay."""
        h, w = frame.shape[:2]
        overlay = frame.copy()
